# Aurora PostgreSQL Global Database – Monitoring & Metrics
...
👉 *Lag affects RPO, data transfer affects cost, and events tell you what failed and when.*
...
### 4. IO Write Count (Primary → Secondary)
...
* If `highest_lsn_received > durable_lsn` → replication is **still catching up**
* If both values are equal → instance is **fully in sync**
...
* Highest LSN > Durable LSN → **secondary is lagging**
...
* **RDS Console → Events** (left navigation)
...
**Replication Lag** and **Progress Lag** — these directly affect RPO and application consistency.
